...
{
  bool unit_clause_found;    // stores whether the current iteration found a unit clause
  if (f.clauses.size() == 0) // if the formula contains no clauses
  {
    return RetVal::satisfied; // it is vacuously satisfied
  }
  do
  {
    ...
    for (int i = 0; i < f.clauses.size(); i++)
    {
      if (f.clauses[i].size() == 1) // if the size of a clause is 1, it is a unit clause
      {
        ...
        if (result == RetVal::satisfied || result == RetVal::unsatisfied)
        {
          return result;
        }
      ...
      {
        return RetVal::unsatisfied; // the formula is unsatisfiable in this branch
      }
    }
  } while (unit_clause_found);
  return RetVal::normal; // if reached here, the unit resolution ended normally
}
