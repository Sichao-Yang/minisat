...
enum RetVal
{
  ...
  normal       // the formula is unresolved so far
};
...
private:
   ...
   * not assigned literal (or decision literal) is represented by -1
   */
  ...
  // to assign a literal with given value, decision level and antecedent
...
void SATSolverCDCL::initialize()
{
  ...
  for (int i = 0; i < clause_count; i++)
  {
    ...
    while (true) // while the ith clause gets more literals
    {
      ...
      {
        clauses[i].push_back(literal); // store it
...
int SATSolverCDCL::unit_propagate(int decision_level)
{
  ...
  int false_count;                // number of false literals in the clause
  int unset_count;                // number of unset literals in the clause
  int literal_index;
  bool satisfied_flag;         // if the clause is satisfied due to the presence of a true literal
  int last_unset_literal = -1; // index of an unset literal
  do
  {
    unit_clause_found = false;
    // iterate over all clauses if no unit clause has been found so far
    for (int i = 0; i < clauses.size() && !unit_clause_found; i++)
    {
      false_count = 0;
      unset_count = 0;
      ...
      for (int j = 0; j < clauses[i].size(); j++)
      {
        ...
        literal_index = variable_to_literal_index(clauses[i][j]);
        if (literals[literal_index] == -1) // if unassigned
        {
          unset_count++;
          last_unset_literal = j; // store the index, may be needed later
        }
        else if ((literals[literal_index] == 0 && clauses[i][j] > 0) ||
                 (literals[literal_index] == 1 && clauses[i][j] < 0)) // if false in the clause
        {
          false_count++;
        }
      ...
      if (unset_count == 1)
      {
        ...
        // the antecedent
        assign_literal(clauses[i][last_unset_literal], decision_level, i);
...
{
  int literal = variable_to_literal_index(variable); // get the index
...
{
  // the new clause to learn, initialized with the antecedent of the conflict (the conflicting clause)
  vector<int> learnt_clause = clauses[kappa_antecedent];
  ...
  do
  {
    ...
    {
      literal = variable_to_literal_index(learnt_clause[i]); // get the index
      ...
      if (literal_decision_level[literal] == conflict_decision_level)
      {
        ...
        if (literal_antecedent[literal] != -1)
        {
  ...
  clauses.push_back(learnt_clause); // add the learnt clause to the list
...
vector<int> &SATSolverCDCL::resolve(vector<int> &input_clause, int literal)
{
  ...
  vector<int> second_input = clauses[literal_antecedent[literal]];
  // concatenate the two
...
{
  if (result_status == RetVal::satisfied) // if the formula is satisfiable
  {
    cout << "SAT" << endl;
    for (int i = 0; i < literals.size(); i++)
    {
